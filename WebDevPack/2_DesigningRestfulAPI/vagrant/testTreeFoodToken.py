...
try:
    ...
    data = dict(name = "peach", category = "fruit", price= "$.99")
...
try:
    h = Http()
    h.add_credentials(token + 'byNico' ,'blank')
    url = address + '/products'
    resp, content = h.request(url,'GET', headers = {"Content-Type" : "application/json"})
    if resp['status'] == '200':
            raise Exception('Received a successful status code of %s' % resp['status'])
except Exception as err:
    print "Test 3 FAILED: End poind to see all products reached with invalid token"
    print err.args
    sys.exit()
else:
    print "Test 4 PASS: Succesfully rejected wrong token"
...
try:
    ...
    print "Test 5 PASS: Succesfully see all products"
...
try:
    ...
    print "Test 6 PASS: Succesfully see a specific category"
